...
#define MIC_GAIN_L     8     // Gain for left channel (mic1, table-facing)
#define MIC_GAIN_R    16     // Gain for right channel (mic2, waiter-facing)
...
static int  currentScreen  = 0;   // 0=waiter 1=rec 2=BLE T1-3 3=BLE T4-5 4=status 5=CPU 6=speedtest 7=logs
...
void recordingTask(void* arg) {
  while (true) {
    ...
    // High-pass filter state (first-order IIR, ~200Hz cutoff at 16kHz)
    // y[n] = alpha * (y[n-1] + x[n] - x[n-1]),  alpha = 0.92
    // Removes low-frequency handling thuds and enclosure vibration
    float hpPrevInL = 0, hpPrevOutL = 0;
    float hpPrevInR = 0, hpPrevOutR = 0;
    const float HP_ALPHA = 0.92f;
    ...
    while (!stopRequested) {
      ...
      if (bytesRead > 0) {
        ...
        for (int i = 0; i < frames; i++) {
          float inL = (float)(int16_t)(rawBuf[i * 2 + 1] >> 16);  // swapped — mic1(SEL=GND) is R on DMA bus
          float inR = (float)(int16_t)(rawBuf[i * 2]     >> 16);  // swapped — mic2(SEL=3V3) is L on DMA bus
          // High-pass filter — attenuates <200Hz (handling thuds, vibration)
          ...
          audioBuf[i * 2]     = (int16_t)constrain((long)(outL * MIC_GAIN_L), -32768, 32767);  // L
          audioBuf[i * 2 + 1] = (int16_t)constrain((long)(outR * MIC_GAIN_R), -32768, 32767);  // R
        }
...
// Screens 0-7, single-click cycles. Hold bar = bottom 2px (y=30, 64px wide).
void updateDisplay() {
  ...
  // ── Hold progress bar ─────────────────────────────────────────────────────
  if (g_btnState == 1 && (currentScreen == 0 || currentScreen == 1 || currentScreen == 6)) {
    ...
    if (currentScreen == 0) oled.drawFastVLine(32, 28, 4, SSD1306_WHITE);
  }
  ...
  if (currentScreen == 0) {
    ...
    if (isRecording && blink) oled.fillCircle(60, 4, 3, SSD1306_WHITE);
    ...
    for (int i = 0; i < 2; i++) {
      ...
      unsigned kb = (unsigned)min((size_t)999, psramFill[i] / 1024);
      snprintf(sbuf, sizeof(sbuf), "B%d:%-3s%3uK", i, state, kb);
...
void setup() {
  ...
  oled.setCursor(0, 25); oled.print("Starting BLE...");
  oled.display();
  delay(1000);
    ...
    if (btnStable == LOW) {
      ...
      if (g_btnState == 1) {
        ...
        if (held >= holdNeeded) {
          ...
          // screens 2, 3, 4, 5, 7: hold does nothing
          g_btnState = 0;
        } else {
