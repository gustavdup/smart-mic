...
// Atomic-safe pending upload counter helpers (cross-core access)
#define PENDING_INC() do { portENTER_CRITICAL_SAFE(&_pend_mux); pendingUploads++; portEXIT_CRITICAL_SAFE(&_pend_mux); } while(0)
#define PENDING_DEC() do { portENTER_CRITICAL_SAFE(&_pend_mux); if (pendingUploads > 0) pendingUploads--; portEXIT_CRITICAL_SAFE(&_pend_mux); } while(0)
#define PENDING_GET() ({ portENTER_CRITICAL_SAFE(&_pend_mux); int _v = pendingUploads; portEXIT_CRITICAL_SAFE(&_pend_mux); _v; })
...
static volatile int pendingUploads = 0;   // files on SD waiting to upload — use PENDING_INC/DEC macros
...
void uploadTask(void* arg) {
  while (true) {
    ...
    while (true) {
      ...
      if (ok) { PENDING_DEC(); addLog("UPL ok"); }
...
void updateDisplay() {
  ...
  if (currentScreen == 0) {
    ...
    { int p = PENDING_GET();
      snprintf(sbuf, sizeof(sbuf), "Queue: %d file%s", p, p == 1 ? "" : "s"); }
