...
#define BUZZER_PIN    4  // Passive buzzer on Seeed Expansion Board v1 (D3/A3 = GPIO4)
...
// Two 1 MB PSRAM buffers. recordingTask writes audio into the active buffer;
...
#define PSRAM_BUF_SIZE  (1050UL * 1024)   // 1 MB — fits 30 s@16kHz/16-bit + BLE chunk with margin
...
bool uploadToMinIO(const char* filename) {
  ...
  uploadKBps = 0.0f; isUploading = false;
  return ok;
}
...
void wifiSpeedTest() {
  ...
  if (cancelled) {
    ...
    u8g2.clearBuffer();
    u8g2.setFont(u8g2_font_5x7_tf);
    u8g2.drawStr(0, 18, "Speedtest");
    u8g2.drawStr(0, 30, "Cancelled");
    u8g2.sendBuffer();
    delay(1500);
    return;
  }
  ...
  while (true) {
    ...
    { SegmentReady seg;
      while (xQueueReceive(segmentQueue, &seg, 0) == pdTRUE) {
        ...
        snprintf(logmsg, sizeof(logmsg), "UPL %s", seg.filename);
        addLog(logmsg);
        ...
        if (ok) {
          ...
          addLog("UPL ok");
        } else { PSRAM_DEC(); writePSRAMToSD(seg); addLog("UPL→SD"); }
      }
    }
...
static void loadPlaceholderWaiter() {
  ...
  u8g2.clearBuffer();
  u8g2.setFont(u8g2_font_5x7_tf);
  u8g2.drawStr(0, 18, "Demo waiter set:");
  u8g2.drawStr(0, 30, "John");
  u8g2.drawStr(0, 40, "d03");
  u8g2.sendBuffer();
  delay(1500);
}
...
static void scanQRCode() {
  ...
  u8g2.clearBuffer();
  u8g2.setFont(u8g2_font_5x7_tf);
  u8g2.drawStr(0, 10, "Camera settling...");
  u8g2.sendBuffer();
  for (int i = 0; i < 20; i++) {
  ...
  u8g2.clearBuffer();
  u8g2.setFont(u8g2_font_5x7_tf);
  u8g2.drawStr(0, 10, "Scan QR code");
  if (waiterName[0]) {
    char wb[34]; snprintf(wb, sizeof(wb), "Now: %s|%s", waiterName, waiterCode);
    u8g2.drawStr(0, 22, wb);
  }
  u8g2.drawStr(0, 34, "Timeout: 30s");
  u8g2.sendBuffer();
  ...
  while (millis() - t0 < 30000 && !found) {
    ...
    if (now2 - lastOled >= 1000) {
      ...
      u8g2.setFont(u8g2_font_5x7_tf);
      u8g2.drawStr(0, 10, "Scan QR code");
      char fb2[24]; snprintf(fb2, sizeof(fb2), "Frames: %d", frameCount);
      u8g2.drawStr(0, 22, fb2);
      char tb[24]; snprintf(tb, sizeof(tb), "Timeout: %ds", secsLeft);
      u8g2.drawStr(0, 34, tb);
      if (waiterName[0]) {
        char wb[34]; snprintf(wb, sizeof(wb), "Now: %s|%s", waiterName, waiterCode);
        u8g2.drawStr(0, 46, wb);
      }
      u8g2.sendBuffer();
    }
  }
  ...
  if (found) {
    tone(BUZZER_PIN, 2000, 120); delay(160);
    tone(BUZZER_PIN, 2800, 80);  delay(100);
    noTone(BUZZER_PIN);
  } else {
  ...
  u8g2.setFont(u8g2_font_5x7_tf);
  if (found) {
    u8g2.drawStr(0, 10, "Waiter set:");
    u8g2.drawStr(0, 22, waiterName);
    u8g2.drawStr(0, 34, waiterCode);
  } else {
    u8g2.drawStr(0, 10, "No QR found");
    u8g2.drawStr(0, 22, "Timed out");
  }
  u8g2.sendBuffer();
  delay(2000);
...
void updateDisplay() {
  ...
  // ── Button hold progress bar — only on screens with hold actions ─────────
  if (g_btnState == 1 && (currentScreen == 0 || currentScreen == 1 || currentScreen == 4)) {
    uint32_t held     = millis() - g_btnStateTime;
    uint32_t holdFull = (currentScreen == 0) ? 6000 : 3000;
    int barW = (int)min((uint32_t)128, held * 128 / holdFull);
    u8g2.drawBox(0, 62, barW, 2);
    // On screen 0 draw a tick mark at the 3s (QR scan) threshold
    if (currentScreen == 0) u8g2.drawVLine(64, 60, 4);  // midpoint = 3s of 6s
  }

  // ── Screen content ────────────────────────────────────────────────────────
  if (currentScreen == 0) {
    // Screen 0: Waiter identity
    u8g2.drawStr(0, 18, "Waiter");
    u8g2.drawHLine(0, 20, 128);
    if (waiterName[0]) {
      u8g2.drawStr(0, 32, waiterName);
      u8g2.drawStr(0, 42, waiterCode);
    } else {
      u8g2.drawStr(0, 32, "No waiter set");
    }
    u8g2.drawStr(0, 58, "3s:scan QR  6s:demo");

  } else if (currentScreen == 1) {
    // Screen 1: Recording status
    char sbuf[26];
    for (int i = 0; i < 2; i++) {
      ...
      else if (activeBuf == i)                state = "idle";
      else if (psramFill[i] > 0)             state = "ready";
      else                                    state = "free";
      snprintf(sbuf, sizeof(sbuf), "B%d: %4uKB  [%-5s]", i, (unsigned)kb, state);
      u8g2.drawStr(0, 18 + i * 10, sbuf);
    }
    if (isUploading && uploadKBps > 0)
      snprintf(sbuf, sizeof(sbuf), "UPL: %.0f KB/s", uploadKBps);
    else
      snprintf(sbuf, sizeof(sbuf), "UPL: idle");
    u8g2.drawStr(0, 38, sbuf);
    { int ps; portENTER_CRITICAL_SAFE(&_pend_mux); ps = pendingSD; portEXIT_CRITICAL_SAFE(&_pend_mux);
      snprintf(sbuf, sizeof(sbuf), "Pend SD: %d", ps);
      u8g2.drawStr(0, 48, sbuf); }
    u8g2.drawStr(0, 58, isRecording ? "Hold 3s: stop rec" : "Hold 3s: start rec");

  } else if (currentScreen == 2) {
    // Screen 2: BLE beacon list
    for (int i = 0; i < NUM_BEACONS; i++) {
      int y = 18 + i * 10;
    ...
    // Screen 3: Overall system status
    char sbuf[24];
    ...
    { int pp, ps; portENTER_CRITICAL_SAFE(&_pend_mux); pp = pendingPSRAM; ps = pendingSD; portEXIT_CRITICAL_SAFE(&_pend_mux);
      snprintf(sbuf, sizeof(sbuf), "Pend: %dP %dSD", pp, ps); }
    u8g2.drawStr(0, 50, sbuf);
    if (waiterName[0]) snprintf(sbuf, sizeof(sbuf), "%s|%s", waiterName, waiterCode);
    else               strncpy(sbuf, "No waiter", sizeof(sbuf));
    u8g2.drawStr(0, 60, sbuf);

  } else if (currentScreen == 4) {
    // Screen 4: Speedtest
    if (!st_done) {
      u8g2.drawStr(0, 18, "Speedtest");
      u8g2.drawStr(0, 30, "Hold 3s to run");
    } else {
      char sbuf[24];
      ...
      u8g2.drawStr(0, 40, sbuf);
      snprintf(sbuf, sizeof(sbuf), "%u KB in %lums", (unsigned)(st_bytes / 1024), st_ms);
      u8g2.drawStr(0, 50, sbuf);
      u8g2.drawStr(0, 60, "Hold 3s: run again");
    }

  } else {
    // Screen 5: Activity log (oldest at top, newest at bottom)
    if (xSemaphoreTake(logMutex, pdMS_TO_TICKS(5)) == pdTRUE) {
...
static const char* loadConfig() {
  ...
  if (fromSD) {
    ...
    if (prefs.isKey("wifi_ssid")) {
      ...
      Serial.printf("[cfg] Loaded from NVS — WiFi:%s  MinIO:%s:%d\n", WIFI_SSID, MINIO_HOST, MINIO_PORT);
      prefs.end();
      return "NVS";
    } else {
...
void setup() {
  ...
  if (strcmp(cfgSrc, "SD") == 0) {
    u8g2.clearBuffer();
    u8g2.setFont(u8g2_font_5x7_tf);
    u8g2.drawStr(0, 10, "Config: SD loaded");
    u8g2.drawStr(0, 22, WIFI_SSID);
    u8g2.drawStr(0, 34, MINIO_HOST);
    u8g2.sendBuffer();
    delay(2000);
  }
...
void loop() {
  ...
  // Hold 3s       → QR scan (screen 0) / start/stop rec (screen 1) / nothing (others)
  // Hold 3s       → run WiFi speedtest (screen 4 only, press to cancel)
  // Hold 6s       → load placeholder waiter (screen 0 only)
      ...
      if (g_btnState == 1) {
        ...
        if (held >= holdNeeded) {
          if (currentScreen == 4) {
            ...
            WiFi.disconnect(false);
            WiFi.mode(WIFI_OFF);
            updateDisplay();  // show results immediately without waiting for next cycle
          } else if (currentScreen == 0) {
            // Hold on waiter screen → scan QR code
            Serial.println("[btn] Scan QR");
            scanQRCode();
          } else if (currentScreen == 1) {
            // Hold on recording screen → start or stop recording
            if (!isRecording) {
              if (!waiterName[0]) {
                // No waiter assigned — prompt to scan first
                u8g2.clearBuffer();
                u8g2.setFont(u8g2_font_5x7_tf);
                u8g2.drawStr(0, 20, "No waiter assigned!");
                u8g2.drawStr(0, 32, "Scan QR first.");
                u8g2.sendBuffer();
                delay(2000);
                currentScreen = 0;
              } else {
          ...
          // screens 2, 3, 5: hold does nothing
          g_btnState = 0;
        } else {
  ...
  // 6s hold on screen 0 auto-fires placeholder waiter
  if (g_btnState == 1 && currentScreen == 0 && millis() - g_btnStateTime >= 6000) {
